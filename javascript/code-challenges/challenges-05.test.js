...
//array = [1,2,3,4]
const addValues = (arr) => {
  let adder = arr.reduce((totals, item) => {
    return totals + item;
  }, 0);
  return adder;
};
...
Write a function named addPurchases that, given an array of objects as input, uses reduce to find the
total amount purchased. Each object contains the keys `item` and `purchasePrice` like the example.

{
  item: 'switch'
  purchasePrice: 399
}
expect(addPurchases([{item: 'switch', purchasePrice: 399}, {item: 'toothpaste', purchasePrice: 2}]))
------------------------------------------------------------------------------------------------ */

const addPurchases = (arr) => {
  let added = arr.reduce((runningTotal, currentNumber) => {
    return runningTotal + currentNumber.purchasePrice;
  }, 0);
  return added;
};
...
const countNumberOfElements = (arr) => {
  let numOfElements = arr.reduce((accumulator) => {
...
const returnNames = (arr) => {
  let newArr = arr.reduce((accumulator, items) => {
    accumulator.push(items.name);
    return accumulator;
  }, []);
  return newArr;
};
...
const countNumberOfChildren = (arr) => {
  let numOfChild = arr.reduce((accumulator, items, index) => {
    return accumulator + items.children;
  }, 0);
  return numOfChild;
};
