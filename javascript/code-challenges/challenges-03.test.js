...
const containsAnd = (arr) => {
  let andArr = arr.filter((idx) => idx.includes('and'));
  return andArr;
};
...
const oddValues = (arr) => {
  let newArr = arr.filter((idx) => (idx % 2));
  return newArr;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let newArr = arr.filter((idx) => !forbiddenValues.includes(idx));
  return newArr;
};
  ...
  stats: [
    ...
    },
    {
      stat: {
        ...
        name: 'special-defense', //NEED THIS
      },
      effort: 2,
      baseStat: 110,
    },
